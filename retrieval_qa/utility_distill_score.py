...
def nliEval(model, tokenizer, premise_raw, hypothesis_raw):
    batch_tokens = tokenizer.batch_encode_plus(list(zip(premise_raw, hypothesis_raw)), padding=True, truncation=True, max_length=512, return_tensors="pt" ,truncation_strategy="only_first")
    with torch.no_grad():
